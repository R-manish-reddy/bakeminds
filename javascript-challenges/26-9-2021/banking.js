...
//   requests_Ops.forEach((req, index) => {
//     let cleanReq = req.replace(/\d+/g, "");
//     var shallContinue =true;

//     switch (cleanReq.trim()) {
//       case "transfer":
//         let transfer_i = parseInt(req.match(/\d+/g)[0]);
//         let transfer_j = parseInt(req.match(/\d+/g)[1]);
//         let transfer_amt = parseInt(req.match(/\d+/g)[2]);

//         if (balStore[transfer_i - 1] < transfer_amt) {
//           console.log("shit",-(index+1));

//         } else {
//           balStore[transfer_i - 1] = balStore[transfer_i - 1] - transfer_amt;
//           balStore[transfer_j - 1] = balStore[transfer_j - 1] + transfer_amt;
//           return console.log(balStore);
//         }

//       case "deposit":
//         let deposit_target = parseInt(req.match(/\d+/g)[0]);
//         let deposit_amt = parseInt(req.match(/\d+/g)[1]);
...
//       case "withdraw":
//         let withdraw_target = parseInt(req.match(/\d+/g)[0]);
//         let withdraw_amt = parseInt(req.match(/\d+/g)[1]);
